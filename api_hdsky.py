    ...
    def sign(self, cookie, Movies_quantity):
        ...
        try:
            ...
            if "欢迎回来" in r.text:
                ...
                user_name = soup.select_one('a.InsaneUser_Name b').text
                Movieslist = soup.select('tr.stickbg.progresstr')
                for i, info in enumerate(Movieslist[:int(Movies_quantity)], start=1):
                    for element in info.select('td.embedded'):
                    ...
                    imdb_element = info.select_one('img[src="/pic/icon-imdb.png"]')
                    douban_element = info.select_one('img[src="/pic/icon-douban.png"]')
                    ...
                    douban_msg = ''
                    if douban_rating:
                        movie_link = info.select_one('a[title]')['href']
                        ...
                        if '<td class="rowhead">豆瓣' in d.text:
                            for element in douban_soup.select('dl#dbdl'):
                    ...
                    name = info.select_one('a[title]')['title']
                    upload_time = info.select('td.rowfollow.nowrap')[1].find('span')['title']

                    td_tags = info.select('td.rowfollow')
